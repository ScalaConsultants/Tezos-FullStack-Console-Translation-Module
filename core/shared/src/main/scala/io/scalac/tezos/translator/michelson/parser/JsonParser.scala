...
import io.scalac.tezos.translator.michelson.parser.JsonParser.EmbeddedElement.toMichelsonElement
...
object JsonParser {
  ...
  case class JsonCodeSection(
    ...
    def toMichelsonCode: MichelsonCode =
      MichelsonCode(
  ...
  object EmbeddedElement {
    def toMichelsonElement(embeddedElement: EmbeddedElement): MichelsonElement =
      embeddedElement match {
        case Left(Left(jsonExpression)) => jsonExpression.toMichelsonExpression
        case Left(Right(jsonInstruction)) =>
          jsonInstruction.toMichelsonInstruction.normalized
